...
public class BitmapUtil {
     ...
     * 返回当前Bitmap图片对象宽度和高度
     *
     * @param bm 要获取尺寸的图片Bitmap对象
     * @return 要返回的图片尺寸信息，例如返回格式为：当前图片宽度和高度分别为@700@500
     */
    public static String getBitmapSize(Bitmap bm) {
        int width = bm.getWidth();
        int height = bm.getHeight();
     ...
     * @param savePath 要保存的文件路径，注意路径后要加/
     */
     ...
     * @param bm       要保存的Bitmap对象
     * @param savePath 要保存的文件路径，注意路径后要加/
     * @param saveName 保存后的图片名称
     */
    public static void saveBitmap(Bitmap bm, String savePath, String saveName) {
        File file = new File(savePath);
        if (!file.exists()) {
            file.mkdir();
        }
     ...
     * @param imageType 要保存为的图片格式，只可输入：png或jpg或jpeg （不区分大小写）
     */
    public static void saveBitmap(Bitmap bm, String savePath, String saveName, String imageType) {
        File file = new File(savePath);
        if (!file.exists()) {
            file.mkdir();
        }
     ...
     *
     * @param bm    要缩小的Bitmap对象
     * @param scale 等比例缩小的尺寸，缩小范围（0.1 - 1.0）;放大范围(1.1 - )
     * @return 返回缩放后的Bitmap对象
     */
    public static Bitmap getBitmapZoom(Bitmap bm, double scale) {
        // if (scale > 1.0) {
        // scale = 1.0;
     ...
     *
     * @param picPath 要处理的图片的路径
     * @param height  要缩小到的图片宽度
     * @return Bitmap对象
     */
    public static Bitmap getBitmapZoom(String picPath, double height) {
     ...
     *
     * @param bm      要处理的图片
     * @param quality 要保存的图片质量（0-100）值越低压缩率越高，不压缩则设为100
     */
